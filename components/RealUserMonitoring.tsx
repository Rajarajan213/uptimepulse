...
function ScoreGauge({ value, label, good, needs }: { value: number; label: string; good: number; needs: number }) {
  ...
  const color = isGood ? '#22c55e' : isNeeds ? '#f59e0b' : '#ef4444'
  const scoreLabel = isGood ? 'GOOD' : isNeeds ? 'NEEDS IMPROVEMENT' : 'POOR'
  const pct = Math.min(100, (value / needs) * 80)

  return (
    <div style={{ background: 'var(--bg-card)', border: `1px solid ${color}30`, borderRadius: 14, padding: 20, textAlign: 'center' }}>
      <div style={{ position: 'relative', width: 90, height: 90, margin: '0 auto 12px' }}>
        <svg viewBox="0 0 90 90" width="90" height="90">
          <circle cx="45" cy="45" r="38" fill="none" stroke="var(--bg-secondary)" strokeWidth="8" />
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
          ...
          <div style={{ fontSize: 8, color: 'var(--text-muted)' }}>ms</div>
        </div>
      </div>
      <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 10, fontWeight: 700, color, background: `${color}15`, borderRadius: 6, padding: '2px 8px', display: 'inline-block' }}>{scoreLabel}</div>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 14 }}>
        {[
          { label: 'Sessions (24h)', val: rum.sessions.toLocaleString(), color: '#3b82f6', icon: '👥' },
          { label: 'Avg Page Load', val: `${rum.pageLoad}ms`, color: '#8b5cf6', icon: '⏱️' },
          { label: 'Bounce Rate', val: `${rum.bounceRate}%`, color: rum.bounceRate > 40 ? '#ef4444' : '#22c55e', icon: '↩️' },
          { label: 'TTFB', val: `${rum.ttfb}ms`, color: rum.ttfb < 600 ? '#22c55e' : '#f59e0b', icon: '🚀' },
        ].map(s => (
      ...
      {/* Core Web Vitals */}
      <div>
        <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, color: 'var(--text-primary)' }}>⚡ Core Web Vitals</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 16 }}>📈 Page Load — 24h</h3>
              ...
              return (
                <div key={h.hour} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{
                    ...
                    background: isHigh ? 'linear-gradient(180deg, #ef4444, #f59e0b)' : 'linear-gradient(180deg, #3b82f6, #8b5cf6)',
                    borderRadius: '4px 4px 0 0',
                    minHeight: 3,
                    transition: 'height 0.5s ease',
                  }} />
                  <div style={{ fontSize: 8, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{h.hour}</div>
                </div>
              )
            })}
          </div>
        ...
        {/* Browser breakdown */}
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 16 }}>🌐 Browser Share</h3>
            ...
            {rum.browsers.map(b => (
              <div key={b.name}>
                ...
                <div style={{ background: 'var(--bg-secondary)', height: 6, borderRadius: 6, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${b.pct}%`, background: b.color, borderRadius: 6, opacity: 0.85 }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', fontSize: 13, fontWeight: 700 }}>🌍 Geographic Distribution</div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: 'var(--bg-secondary)' }}>
              {['Country', 'Sessions', 'Avg Load', 'Score'].map(h => (
                <th key={h} style={{ padding: '10px 20px', textAlign: 'left', fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rum.geoData.map((row, i) => (
              <tr key={row.country} style={{ borderTop: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                <td style={{ padding: '12px 20px', fontSize: 13, color: 'var(--text-primary)' }}>{row.flag} {row.country}</td>
                <td style={{ padding: '12px 20px', fontSize: 13, color: 'var(--text-secondary)' }}>{row.sessions.toLocaleString()}</td>
                <td style={{ padding: '12px 20px', fontSize: 13, color: 'var(--text-secondary)' }}>{row.avgLoad}</td>
                <td style={{ padding: '12px 20px' }}>
                  <span style={{
                    fontSize: 11, fontWeight: 700,
                    color: row.score === 'Good' ? '#22c55e' : row.score === 'Needs Improvement' ? '#f59e0b' : '#ef4444',
                    background: row.score === 'Good' ? 'rgba(34,197,94,0.1)' : row.score === 'Needs Improvement' ? 'rgba(245,158,11,0.1)' : 'rgba(239,68,68,0.1)',
